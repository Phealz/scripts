...
function main()
    ...
    while not isSampAvailable() do wait(100) end
        ...
        sampAddChatMessage(tagG .. " {890303}by Phealz {00C22A}loaded.{FFFFFF} Version: {890303}v.2.0 ", -1)
    ...
    while true do
        ...
        if main_window_state.v or oscripte.v or osnfunctions.v or piarik.v or chasiki.v or autologin.v or infob.v then
...
function obinfo(arg)
    ...
    sampAddChatMessage(tagG .. ' {FFFFFF}Выполнен выход из беты, добавлены новые функции и изменён интерфейс.', -1)
    sampAddChatMessage(tagG .. ' {FFFFFF}Удачной игры, '..nick, id, -1)
end

function imgui.OnDrawFrame()
    if main_window_state.v then
		imgui.SetNextWindowSize(imgui.ImVec2(196, 153), imgui.Cond.FirstUseEver)
        ...
        if oscripte.v then
            ...
            imgui.Begin(u8'О скрипте', oscripte, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove)
                ...
                if imgui.Button(u8'Информация по обновлению', imgui.ImVec2(533, 35)) then
                    infob.v = true
                end

        if infob.v then
            imgui.SetNextWindowPos(imgui.ImVec2(ScreenX / 2 , ScreenY / 2), imgui.Cond.FirsUseEver, imgui.ImVec2(0.742, 0.89))
            imgui.Begin(u8'Информация по обновлению', oscripte, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove)
                imgui.SetCursorPosX(20)
                imgui.TextColoredRGB('{FFFFFF}1. Добавлен новый {c70e26}интерфейс.')
                imgui.SetCursorPosX(20)
                imgui.TextColoredRGB('{FFFFFF}2. Добавлена функция {c70e26}АвтоЛогина.')
                imgui.SetCursorPosX(35)
                imgui.TextColoredRGB('{FFFFFF}Удачной игры, {c70e26}'..nick)
            imgui.End()
        end
            imgui.End()
        end
